...
style.textContent='@import url("https://fonts.googleapis.com/css2?family=Orbitron:wght@700;900&family=Rajdhani:wght@400;600&display=swap");#_akg_overlay{position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:2147483647;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(10px);}#_akg_box{background:#0d0d1a;border-radius:20px;padding:28px 24px 20px;width:320px;max-width:94vw;text-align:center;font-family:"Rajdhani",sans-serif;position:relative;border:2px solid transparent;background-image:linear-gradient(#0d0d1a,#0d0d1a),radial-gradient(circle at top left,#ff0000,#00ff00,#0000ff,#ff0000);background-origin:border-box;background-clip:padding-box,border-box;animation:rgb-border 4s linear infinite;box-shadow:0 0 25px rgba(108,71,255,0.3);}@keyframes rgb-border{0%{background-image:linear-gradient(#0d0d1a,#0d0d1a),radial-gradient(circle at top left,#ff0000,#00ff00,#0000ff);}33%{background-image:linear-gradient(#0d0d1a,#0d0d1a),radial-gradient(circle at top left,#00ff00,#0000ff,#ff0000);}66%{background-image:linear-gradient(#0d0d1a,#0d0d1a),radial-gradient(circle at top left,#0000ff,#ff0000,#00ff00);}100%{background-image:linear-gradient(#0d0d1a,#0d0d1a),radial-gradient(circle at top left,#ff0000,#00ff00,#0000ff);}}#_akg_title{font-family:"Orbitron",sans-serif;font-size:15px;font-weight:900;letter-spacing:3px;text-transform:uppercase;margin-bottom:2px;animation:rgb-text 4s linear infinite;}@keyframes rgb-text{0%{color:#ff0000;text-shadow:0 0 10px #ff0000;}33%{color:#00ff00;text-shadow:0 0 10px #00ff00;}66%{color:#0000ff;text-shadow:0 0 10px #0000ff;}100%{color:#ff0000;text-shadow:0 0 10px #ff0000;}}#_akg_sub{font-size:12px;color:#888;letter-spacing:1px;margin-bottom:20px;}#_akg_inp_wrap{position:relative;margin-bottom:14px;}#_akg_inp{width:100%;background:#0a0a18;border:1.5px solid #6c47ff55;border-radius:10px;padding:12px 16px;color:#fff;font-size:16px;font-family:"Orbitron",sans-serif;letter-spacing:4px;text-align:center;outline:none;box-sizing:border-box;transition:border .3s;}#_akg_inp:focus{border-color:#6c47ff;}#_akg_inp::placeholder{color:#333;letter-spacing:2px;font-size:13px;}#_akg_btn{width:100%;background:linear-gradient(135deg,#6c47ff,#a855f7);border:none;border-radius:10px;padding:13px;color:#fff;font-family:"Orbitron",sans-serif;font-size:12px;font-weight:700;letter-spacing:2px;cursor:pointer;transition:opacity .2s;margin-bottom:16px;box-shadow:0 0 15px rgba(108,71,255,0.3);}#_akg_btn:hover{opacity:.85;transform:scale(1.02);}#_akg_err{color:#f87171;font-size:12px;margin-bottom:10px;min-height:16px;letter-spacing:1px;}#_akg_circle_wrap{display:none;flex-direction:column;align-items:center;margin-bottom:16px;}#_akg_status{font-size:12px;color:#00f2fe;letter-spacing:1px;margin-top:12px;font-weight:bold;}#_akg_tg{display:flex;align-items:center;justify-content:center;gap:6px;color:#fff;font-size:11px;text-decoration:none;letter-spacing:1px;transition:color .2s;margin-top:4px;padding:5px;background:rgba(24,119,242,0.1);border-radius:8px;}#_akg_close{position:absolute;top:14px;right:16px;color:#333;font-size:18px;cursor:pointer;line-height:1;}#_akg_close:hover{color:#ff0000;}';
...
function bypassSite(domain,cb){
  // আপনার অরিজিনাল কোডের প্রোটোকল লজিক ফিরিয়ে আনা হয়েছে
...
btn.onclick=function(){
  ...
  if(val!=='SAJIBX'){err.textContent='\u274c Wrong key! Access denied.';inp.value='';return;}
  err.textContent='';
  if(h.includes('tarviral.com')||h.includes('rodaemotor.com')){
    ...
    err.textContent='\u26a0 Open the correct site first!';
  }
};
